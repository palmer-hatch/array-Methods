...
const first = (arr, callback) => callback (names[0])
...
const uniq = (arr,callback) => {
  const newNameSet = new Set(arr);
  let newArr = [...newNameSet]
  callback (newArr)
    
  }

  uniq(names, newNameSet => {
    console.log(`The new names array with all the duplicates removed is ${newNameSet}`)})
